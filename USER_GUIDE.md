...
- **First-time setup** — When you open the app and no vault exists yet, you see a **Create your vault** form: choose a **username** and **master password** (and confirm the password), then click **Create vault**. You are then logged in. No command line required.
- **Multi-user** — Each user has their own username, password, saved folders/entries, and recovery settings. After the first user is created, others can click **Create account** on the login page.
- **Unlock / lock** — You enter your **username** and **master password** to open the vault; the app remembers you're logged in until you click Lock or leave it idle for 15 minutes.
...
2. If the vault has not been set up yet, you will see **"First time here?"** with a short explanation and fields: **Username**, **Choose master password**, and **Confirm master password**.
3. Enter a **username** (e.g. your name or email) and a **master password** in both password fields (they must match). You will use this username and password every time you log in.
...
For a **line-by-line and block-by-block explanation** of the code, see **[EDUCATIONAL_CODE_WALKTHROUGH.md](EDUCATIONAL_CODE_WALKTHROUGH.md)**. For the full documentation index, see **[DOCS.md](DOCS.md)**.
